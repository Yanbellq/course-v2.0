import Button from"../../../dist_ts/components/btn.js";document.addEventListener("DOMContentLoaded",function(){document.querySelector("#product-list-table")&&(console.log("Initializing product list page..."),loadProducts())});let btn_props={text:"Додати товар",link:"add",styles:"shadow-glow",icon:"plus",icon_styles:""},pageHeaderActions=document.querySelector("#page-header-actions");async function loadProducts(){var e=document.querySelector("#product-list-body");document.querySelector("#product-list-table");if(e){e.innerHTML=`
        <tr>
    ...
    `;try{renderProducts((await PCManagement.apiRequest("/products/")).products,e),PCManagement.initDataTable("#product-list-table",{pageSize:15})}catch(t){console.error("Failed to load products:",t),e.innerHTML=`
        ...
        `}pageHeaderActions&&pageHeaderActions.insertAdjacentHTML("beforeend",Button(btn_props));